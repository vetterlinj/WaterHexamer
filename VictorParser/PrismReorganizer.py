...
def bigOne2(path,dataname,mostlyD,allH,simulation):
    ...
    #    writeMeAnXYZFile(coords[1],path+dataname+'.xyz',deuterium)
        ...
        if deuterium==18:
            np.savez(f'SortedData/{resultspath}' + dataname+'_'+f'{coordsnumber}', coords=coords, weights=weights)
        else:
            np.savez(f'SortedData/{resultspath}' + dataname+f'_{deuterium}'+f'_{coordsnumber}', coords=coords, weights=weights)


for simulation in np.arange(1, 7):
    path=f'h2o5_d2o_prism/minimum{simulation}_wfns/PythonData/'
    # path = f'h2o_d2o5_prism/minimum{simulation}_wfns/PythonData/'
    # path = f'h2o6_prism/PythonData/'
    dataname = f'h2o5_d2o_prism'
    mostlyD = False
    allH=False
    bigOne2(path,dataname,mostlyD,allH,simulation)
