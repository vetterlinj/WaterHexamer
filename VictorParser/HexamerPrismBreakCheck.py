...
for simulation in np.arange(2,5):
    ...
    mostlyD=True
    fulldata,weights,metadata,size=concatenatenpz(path+'FullDataset/')
    ...
    for a in np.arange(0,length):
        originwfn=int(origins[a,0,0])-1
        originwalker=int(origins[a,0,1])-1
        origincount=0
        for wfn in np.arange(0,originwfn):
            origincount+=size[wfn]
        origincount+=originwalker
        reference.append(origincount)
        weight=weights[origincount]
        haserror=0
    ...
    for b in waterlist:
        for c in np.arange(0, 3):
            frencharray[:, valuecounter, :] =fulldata[:, b + c, :]
            valuecounter = valuecounter + 1
    frencharray = np.array(frencharray)
    coords = frencharray
    # writeMeAnXYZFile(coords[0],path+f'whatisgoingon{simulation}.xyz',deuterium)
    # continue
    # exit()
    results = {
        'oneToThreeCount': 0,
        ...
        'seventeenToSixCount': 0,
        'confusedCount': 0}
    resultsFile = open(path + "UnOptResults.txt", 'w')
    count = 0
    for a in reference:
        weight = weights[a]
        haserror = 0
