...
for weirdsimthingy in np.arange(0,1):
    ...
    for i in np.arange(0,5):
        if bunchofjobs == True:
            ...
            arg5= sys.argv[5]
            massfactor=float(arg5)
            nameMassFactor = str(massfactor).replace(".", "point")
            #need fifth argument
            filename="Results/Multiple/Size/"+fnameExtension+f"_{nameMassFactor}_{i}"
            print(filename)
            resultsfilename="Results/Multiple/Size/npzFiles/"+fnameExtension+f"_{nameMassFactor}_{i}"
        dimensions=3
        ...
        #mass for each coord
        def randommovement(coords,dimensions,deltaTau,massfactor):
            count=0
            ...
            def birthandDeath(coords, energies, alpha, numWalkers,weights,deltaTau):
                ...
                if deathcount>0:
                    for neededwalker in np.arange(0,deathcount):
                        ...
                        birthlist.append(deletedcoords[actualwalker + 1])
                        birthlist.append(deletedcoords[actualwalker + 2])
        ...
        for i in np.arange(0,numTimeSteps):
            ...
            if count %100 ==0:
                print(count)
            coords=randommovement(coords,dimensions,deltaTau, massfactor)
