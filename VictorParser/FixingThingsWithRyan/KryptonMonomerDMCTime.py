...
for weirdsimthingy in np.arange(0,1):
    ...
    for i in np.arange(0,5):
        if bunchofjobs == True:
            ...
            filename="Results/Multiple/Time/"+fnameExtension+f"_{namedeltaTau}_{i}"
            print(filename)
            resultsfilename="Results/Multiple/Time/npzFiles/"+fnameExtension+f"_{namedeltaTau}_{i}"
        dimensions=3
        ...
        if DescWeights==False:
            ...
            def birthandDeath(coords,energies,alpha,numWalkers,weights,deltaTau):
                ...
                print("Number Killed: " + str(len(deathlist) / 3))
                if len(birthlist)==0:
                    print('empty')
                    birthedcoords=deletedcoords
                else:
                    print(len(birthlist) / 3)
