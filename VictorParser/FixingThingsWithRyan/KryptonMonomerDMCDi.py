...
for weirdsimthingy in np.arange(0,1):
    ...
    for i in np.arange(0,5):
        if bunchofjobs == True:
            ...
            filename="Results/Multiple/DiTime/"+fnameExtension+f"_{namedeltaTau}_{i}"
            print(filename)
            resultsfilename="Results/Multiple/DiTime/npzFiles/"+fnameExtension+f"_{namedeltaTau}_{i}"
        dimensions=3
        ...
        def getDemEnergies(coords):
            ...
            energies=np.add.reduceat(listenergies, np.arange(0,len(listenergies),2))
            return energies
        if DescWeights==False:
            def getVref(energies,alpha,weights,numWalkers,coords):
                Vref = np.average(energies) - (alpha / numWalkers * ((len(coords)/3)/2 - numWalkers))
                return Vref
            def birthandDeath(coords,energies,alpha,numWalkers,weights,deltaTau):
                averageEnergy=np.average(energies)
                # print('averageEnergy:')
                # print(averageEnergy)
                Vref=averageEnergy-(alpha/numWalkers*(len(coords[:,0])/6-numWalkers))

                # print(len(coords[:,0]))
                # print()
                birthlist=[]
                deathlist=[]
                for walker in np.arange(0,int(len(coords[:,0])/6)):
                    random=np.random.random()
                    if energies[walker]<Vref:
                        prob=np.exp(-deltaTau*(energies[walker]-Vref))-1
                        if random<prob:
                            actualwalker=walker*6
                            for ff in np.arange(0,6,1):
            ...
            def birthandDeath(coords, energies, alpha, numWalkers,weights,deltaTau):
                ...
                deathweights=[]
                deathcount=0
                for walker in np.arange(0, int(len(weights))):
                    expon=np.exp((0-deltaTau) * (energies[walker] - Vref))
        ...
        coords=np.zeros((numWalkers*6,dimensions))
        weights=np.ones(numWalkers)
        for i in np.arange(0,numWalkers*2):
