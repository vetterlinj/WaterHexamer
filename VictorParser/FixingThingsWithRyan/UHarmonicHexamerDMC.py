...
for weirdsimthingy in np.arange(0,1):
    ...
    # omega=1
    omega = 3893.442128 * (4.5563e-6)
    wexe = 74.9406968 * (4.5563e-6)
    k = m * (omega ** 2)
    ...
    for i in np.arange(0,1):
        if bunchofjobs == True:
            filename = "Results/Multiple/Harmonic/Cage/" + fnameExtension + f"_{namedeltaTau}_{i}"
            resultsfilename = "Results/Multiple/Harmonic/Cage/npzFiles/" + fnameExtension + f"_{namedeltaTau}_{i}"
        ...
        np.savez(resultsfilename+"Data",energies=fullEnergies,coords=coords,iwalkers=numWalkers,deltatau=deltaTau,timesteps=numTimeSteps,weights=weights,omega=omega,wexe=wexe)
